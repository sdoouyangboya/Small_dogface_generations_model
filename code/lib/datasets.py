...
def get_caption(cap_path,clip_info):
    eff_captions = []
   # Open file with UTF-8 encoding, ignoring problematic characters
...
class TextImgDataset(data.Dataset):
    ...
    def load_filenames(self, data_dir, split):
        filepath = '%s/%s/filenames.pkl' % (data_dir, split)
    ...
    def __getitem__(self, index):
        ...
        if self.bbox is not None:
            ...
            bbox = None
        #
        if self.dataset_name.lower().find('dog') != -1:
            if self.split=='train':
                img_name = '%s/imgs/train/%s.png' % (data_dir, key)
                text_name = '%s/text/%s.txt' % (data_dir, key)
            else:
                img_name = '%s/imgs/val/%s.png' % (data_dir, key)
                text_name = '%s/text/%s.txt' % (data_dir, key)
